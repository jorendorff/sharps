...
Narcissus.parser = (function() {
    ...
    function PrimaryExpression(t, x) {
        ...
        switch (tt) {
          ...
          case SHARPDEF:
            if (t.peek(true) !== LEFT_CURLY && t.peek(true) !== LEFT_BRACKET)
                throw t.newSyntaxError("expected object literal or array literal after sharp variable definition");
            n = new Node(t, { type: SHARPDEF, id: t.token.value });
            n.push(PrimaryExpression(t, x));
            break;

          case SHARPREF:
            n = new Node(t, { type: SHARPREF, id: t.token.value });
            break;
